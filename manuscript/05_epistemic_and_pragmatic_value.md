...
allocation) by its **Expected Free Energy**, which in Active Inference decomposes
...
- **Epistemic value** — how much the action is expected to *reduce uncertainty
  ...
  channels — they sharpen the model that prices all future outcomes.

The marginal-return vector is then $g_t = -\partial G/\partial a_t$, and at the
optimum every funded channel equates the same risk-adjusted shadow price of
capital — AlphaFund's equimarginal identity $\hat g^k_t/\sigma^k_t =
...
funded policy: pragmatic {{FUNDED_PRAGMATIC}}, epistemic {{FUNDED_EPISTEMIC}}, for
the channel it selects ({{FUNDED_CHANNEL}}) at the model's current operating point.
...
horizon**, matching AlphaFund's "path integral along the planned allocation path,"
...
*modestly negative*. That is the correct behavior of an honest instrument on a
coarse encoding, not a defect: the two-level reduction lacks the dynamic range to
...
rather than promoting drift on noise. AlphaFund's claim that "the certificate gates
...
than baking in a significance level.
