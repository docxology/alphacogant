...
The literature on recursive self-improvement (RSI) — from Yudkowsky's seed AI [13],
through Schmidhuber's Gödel Machines [12], to intelligence-explosion dynamics and
diminishing-returns analyses [12, 13] — often studies self-improvement where
...
problem [1, 24, 25]. Every FLOP and every bit of data costs money; a system that
...
survival test and an auditable certificate [1, 4]. RSI is therefore a
**stochastic control problem under a survival constraint** [1, 24, 25], and a corporation is
its cleanest instance: a legal object with perpetual succession that turns capital into
improved capability and improved capability back into capital [18].
...
structure, not just return maximization, determines what a firm can credibly fund [14].
...
discounted log-return on shareholders' equity subject to solvency [9]. The controller is
a model-predictive convex program over an **Economic World Model (EWM)** — a learned,
filtration-respecting approximation to the true corporate transition law [1, 25]. The
...
information — is an **Active Inference** agent [2, 3, 4, 5, 25]. AlphaFund's
...
each admissible funding move [3, 4]. The two posteriors whose separation defines
...
1. **Renders the firm as a generative model in GNN.** Generalized Notation
   ...
   its epistemic and pragmatic parts [5]. The GNN pipeline is explicit about
   typed parsing, validation, rendering, and executable exports, so the model is
   readable and checkable before inference [5]. In this representation, the firm's
...
2. **Produces that model by the COGANT pattern.** COGANT is a codebase-to-GNN
   ...
   provenance check [6]. The differentiable corporation is a system whose every
   operational degree
   ...
   use the COGANT translation step in miniature [6] to map a firm description onto
...
3. **Computes inference, value, and the certificate.** A small, deterministic,
   ...
   and pragmatic value [3, 4], derives the marginal-return vector, and evaluates
...
corpus [1]. Active Inference is built on this discipline: the generative model
...
resolved later [2, 3, 4, 24, 25]. This is where COGANT and the GNN graph contract are doing
most of the heavy lifting [5, 6]. GNN makes this factorization explicit and
checkable [5].
The "no-peeking" property AlphaFund must bolt onto a wrapped LLM is the native
semantics of the object COGANT emits [6, 25]. This is the same hard constraint that
separates time-aware economic forecasting from post-fitted narrative claims [1, 2,
3, 4, 24].
...
future outcomes (epistemic) [3, 4]. The chronic difficulty of comparing an
explore dollar to an exploit dollar dissolves into the same quantity Active
Inference has formalized for the past decade as a single value decomposition [2, 3, 4, 23].
...
{{PLANNING_HORIZON}}-cycle horizon [24]. [@fig:loop] renders that loop with each
stage labelled by its engine symbol, so the reader can see the whole correspondence
before the formalism arrives [2, 3, 4, 25].
