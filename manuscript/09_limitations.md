...
$\{\text{weak}, \text{strong}\}$. This is a deliberate legibility choice — it makes
the factor graph readable, the inference exact, and the EFE decomposition
transparent — but it pays a measurable cost: the reduced model lacks the dynamic
range to *robustly* certify net self-improvement under belief uncertainty. The
headline t-RSI of {{HEADLINE_T_RSI}} standardised units is modestly negative at
the self-improving operating point, not because the firm is not
...
equimarginal identity in principle, but it cannot represent a portfolio that
...
updates. In the real corporation, the EWM itself (the $\Theta$ factor's
...
transition probabilities — would make the self-forecasting loop endogenous. The
current model captures the *incentive* to refit (epistemic value of funding
...
observation model. The mean-field approximation is exact for the current factor
graph but would become variational under coupling.
...
a function of cumulative pragmatic value — would close this loop.
